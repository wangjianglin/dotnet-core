...
using AD.Core.Web.Http;
using AD.Core.Web.Packages;
...
namespace AD.Core.DataValidation
{
    ...
    {
        public static void AddValidation(AD.Core.ViewModel.ViewModel obj,ValidationErrorData error,string name="")
        {
            ...
            foreach(PropertyInfo py in pInfo)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        if (pyType == typeof(AD.Core.ViewModel.ViewModel) || pyType == typeof(AD.Core.ViewModel.ViewModelBase) || pyType == typeof(AD.Core.ViewModel.ViewModelProperty))
                        {
                            continue;
                        }
                        ...
                        if (o != null)
                        {
                            AddValidation(o as AD.Core.ViewModel.ViewModel, errorTmp, name + py.Name + ".");
                        }
                    }
                    catch (Exception) { }
                }
        ...
        public static void AddValidation(Package package, AD.Core.Web.Http.ValidationErrorData error)
        {
            IDictionary<string, object> param = package.GetParams();
            foreach (KeyValuePair<string,object> vp in param)
            {
                AddValidation(vp.Value as AD.Core.ViewModel.ViewModel, error, vp.Key + ".");
            }
        }
    }
}
